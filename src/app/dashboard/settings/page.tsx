...
export default function SettingsPage() {
    ...
    const fetchSettings = useCallback(async (signal?: { cancelled: boolean }) => {
        ...
        const { data: business } = await supabase
            .from('businesses')
            .select('id, sms_template, sms_template_closed, timezone, business_hours, repairdesk_store_url, business_phone, owner_phone, carrier, forwarding_number, auto_reply_enabled, daily_digest_enabled, status_updates_enabled, followup_auto_send, booking_url, google_review_link')
            .eq('user_id', user.id)
            .single();
